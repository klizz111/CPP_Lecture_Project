...
std::string addStrings(const std::string &num1, const std::string &num2) {
    auto split = [](const std::string &num) {
        size_t pos = num.find('.');
        if (pos == std::string::npos) {
            return std::make_pair(num, std::string(""));
        } else {
            return std::make_pair(num.substr(0, pos), num.substr(pos + 1));
        }
    };

    auto addIntegerStrings = [](const std::string &int1, const std::string &int2) {
        std::string result;
        int carry = 0, sum = 0;
    ...
    auto addFractionalStrings = [](const std::string &frac1, const std::string &frac2) {
        std::string result;
        int carry = 0, sum = 0;
        int i = frac1.size() - 1, j = frac2.size() - 1;

        while (i >= 0 || j >= 0 || carry) {
            sum = carry;
            if (i >= 0) sum += frac1[i--] - '0';
            if (j >= 0) sum += frac2[j--] - '0';
            carry = sum / 10;
            result.push_back(sum % 10 + '0');
        }
    ...
    auto [int1, frac1] = split(num1);
    auto [int2, frac2] = split(num2);

    std::string intResult = addIntegerStrings(int1, int2);
    std::string fracResult = addFractionalStrings(frac1, frac2);

    if (!fracResult.empty() && fracResult.back() == '0') {
        fracResult.pop_back();
    }
...
std::string subtractStrings(const std::string &num1, const std::string &num2) {
    auto splitNumber = [](const std::string &num) {
        size_t pos = num.find('.');
        if (pos == std::string::npos) {
            return std::make_pair(num, std::string("0"));
        } else {
            return std::make_pair(num.substr(0, pos), num.substr(pos + 1));
        }
    };

    auto padZeros = [](std::string &num, size_t newSize, bool padFront) {
        if (num.size() < newSize) {
            if (padFront) {
                num.insert(num.begin(), newSize - num.size(), '0');
            } else {
                num.append(newSize - num.size(), '0');
            }
        }
    };

    auto removeTrailingZeros = [](std::string &num) {
        while (num.size() > 1 && num.back() == '0') {
            num.pop_back();
        }
    };

    auto [intPart1, fracPart1] = splitNumber(num1);
    auto [intPart2, fracPart2] = splitNumber(num2);

    padZeros(fracPart1, std::max(fracPart1.size(), fracPart2.size()), false);
    padZeros(fracPart2, fracPart1.size(), false);

    padZeros(intPart1, std::max(intPart1.size(), intPart2.size()), true);
    padZeros(intPart2, intPart1.size(), true);

    std::string n1 = intPart1 + fracPart1;
    std::string n2 = intPart2 + fracPart2;
    ...
    while (i >= 0 || j >= 0) {
        diff = (i >= 0 ? n1[i--] - '0' : 0) - (j >= 0 ? n2[j--] - '0' : 0) - borrow;
        if (diff < 0) {
            diff += 10;
            borrow = 1;
        } else {
    ...
    result.insert(result.end() - fracPart1.size(), '.');
...
{
    auto n1 = std::stod(num1);
    auto n2 = std::stod(num2);
    if (n2 == 0) {
...
std::string stringPow(const std::string &num1, const std::string &num2)
{
    auto n1 = std::stod(num1);
    auto n2 = std::stod(num2);
    return std::to_string(std::pow(n1, n2));
}

std::string stringSqrt(const std::string &num1, const std::string &num2)
{
    auto n1 = std::stold(num1);
    auto n2 = std::stold(num2);
    auto res = std::sqrt(n1 + n2);
